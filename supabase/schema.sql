...
alter table public.trades alter column fees set default 0;
...
begin
  ...
  if not exists (select 1 from pg_constraint where conname = 'trades_fees_non_negative') then
    alter table public.trades
      add constraint trades_fees_non_negative check (fees >= 0) not valid;
  end if;
