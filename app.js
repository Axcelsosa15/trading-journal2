...
(function () {
  ...
  // `format` overrides how a genuinely ambiguous D/M/Y-vs-M/D/Y date (both parts
  // <=12, e.g. "03/04/2026") is read: "dmy" forces day-first, "mdy" forces
  // month-first (the common US broker/prop-firm export format), "auto" (default)
  // keeps the previous best-guess heuristic. Unambiguous dates (either part >12)
  // are unaffected by `format` — there's only one valid reading.
  function importDate(v, format) {
    var s = String(v || "").trim();
    ...
    if (m) {
      ...
      else if (format === "mdy") { day = b; mon = a; }     // ambiguous, user said month-first
      else { day = a; mon = b; }                           // ambiguous, default/"dmy": day-first
  ...
  function buildImportRow(cells, map, dateFormat) {
    function get(k) { var i = map[k]; return i != null && i >= 0 ? cells[i] : ""; }
    var date = importDate(get("date"), dateFormat);
  ...
  function importPreview() {
    ...
    im.rows.slice(1).forEach(function (cells, n) {
      var r = buildImportRow(cells, im.map, im.dateFormat);
      if (r.row) {
  ...
  // Summing balances across accounts only makes sense when they share one
  // currency; mixing e.g. USD and EUR into one number would misrepresent the
  // total. Sum only the majority currency and surface how many accounts (and
  // which currency) were left out, instead of silently fabricating a total.
  function acctBalanceInfo() {
    ...
    accs.forEach(function (a) {
      ...
      sums[c] = (sums[c] || 0) + (Number(a.balance) || 0);
