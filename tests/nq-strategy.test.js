...
setTimeout(()=>{try{
  // Settings: strategy card + load checklist
  click([...d.querySelectorAll("aside nav button")].find(b=>/Ajustes/.test(b.textContent)));
  const main=d.querySelector("main").textContent;
  console.log("NQ strategy card present:", /Sistema NQ · EMA\/VWAP/.test(main));
  const ta1=d.querySelector("main textarea");
  console.log("Strategy checklist not yet loaded:", !/EMA200/.test(ta1.value));
  const loadBtn=[...d.querySelectorAll("main button")].find(b=>/Añadir checklist del sistema/.test(b.textContent));
  console.log("Load-checklist button present:", !!loadBtn);
  click(loadBtn);
  const ta2=d.querySelector("main textarea");
  console.log("Checklist now includes EMA/VWAP checks:", /EMA200/.test(ta2.value)&&/VWAP/.test(ta2.value));
  // idempotent: clicking again doesn't duplicate
  const before=(ta2.value.match(/EMA200/g)||[]).length;
  click([...d.querySelectorAll("main button")].find(b=>/Añadir checklist del sistema/.test(b.textContent)));
  const after=(d.querySelector("main textarea").value.match(/EMA200/g)||[]).length;
  console.log("Load is idempotent (no duplicates):", before===1&&after===1);
  // Add modal: EMA/VWAP available as a Setup
  ...
  console.log("EMA/VWAP available as a Setup:", opts.indexOf("EMA/VWAP")>=0);
  console.log("NQ STRATEGY SMOKE OK");
