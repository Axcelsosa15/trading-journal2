...
console.log("auth sends captchaToken to Supabase:", /options\s*=\s*\{\s*captchaToken:\s*captchaToken\s*\}/.test(app) && /signInWithPassword\(authRequest/.test(app) && /signUp\(authRequest/.test(app));
console.log("auth renders CAPTCHA widget when configured:", /id:\s*"auth-captcha"/.test(app) && /api\.render\(box/.test(app));
console.log("auth explains missing CAPTCHA site key:", /captchaConfigMissingMessage/.test(app) && /site key publica|site key pública/i.test(app));
