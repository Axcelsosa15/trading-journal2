...
# Browsers reject `allow_credentials=True` together with a wildcard origin. This app
# authenticates via a Bearer token (Authorization header), not cookies, so credentials
...
async def startup():
    # Fail loud on insecure defaults so they're not silently shipped to production.
    if os.environ.get("JWT_SECRET", "change-me") == "change-me":
        logger.warning(
            "JWT_SECRET is using the insecure default. Set a strong JWT_SECRET env var in production."
        )
    if not os.environ.get("ENCRYPTION_KEY"):
        logger.warning(
            "ENCRYPTION_KEY is not set. Stored broker API keys will become undecryptable after a "
            "restart. Generate one with: python -c \"from cryptography.fernet import Fernet; "
